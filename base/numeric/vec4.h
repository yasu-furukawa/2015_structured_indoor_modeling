...
class TVec4 {
 ...
 public:
  ...
  inline void sort(void) {
    std::sort(m_elt, m_elt + 4);
  }
  
  inline void rotate(void) {
    const T minv = min(m_elt[0], min(m_elt[1], min(m_elt[2], m_elt[3])));
    if (m_elt[0] == minv)
      return;
    else if (m_elt[1] == minv) {
      const T ttmp = m_elt[0];
      m_elt[0] = m_elt[1];
      m_elt[1] = m_elt[2];
      m_elt[2] = m_elt[3];
      m_elt[3] = ttmp;
    }
    else if (m_elt[2] == minv) {
      const T ttmp0 = m_elt[0];
      const T ttmp1 = m_elt[1];
      m_elt[0] = m_elt[2];
      m_elt[1] = m_elt[3];
      m_elt[2] = ttmp0;;
      m_elt[3] = ttmp1;
