...
DEFINE_int32(label_num,5000,"Number of superpixel");
...
int main(int argc, char **argv){
  ...
  for (int id=startid; id<startid+1; id++) {
    ...
    if(!labelin.is_open()){
      cout<<"Superpixel file doesn't exist!"<<endl;
      exit(-1);
    }
    ...
    vector <bool> pixelLabel(imgwidth * imgheight);
    vector <bool> superpixelConfidence(numlabels);
