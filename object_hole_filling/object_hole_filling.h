...
void MRFOptimizeLabels_multiLayer(const std::vector< std::vector<double> >&superpixelConfidence, const std::map<std::pair<int,int>,int> &pairmap, const std::vector< Eigen::Vector3d> &averageRGB,const structured_indoor_modeling::DepthFilling &depth, float smoothweight, int numlabels, std::vector <int> &superpixelLabel);
